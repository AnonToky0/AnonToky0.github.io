...
### MTU
- MTU，Maximum Transmission Unit，最大传输单元，定义了一个数据包在单次传输中，所能携带的有效数据载荷的最大尺寸(单位：字节)。
- MSS, Maximum Segment Size，最大报文段长度，定义了一个TCP数据段中能够包含的应用层数据的最大长度(单位：字节)。
- MSS = MTU - IP包头长度 - TCP包头长度

MTU是个本地概念：MTU是针对一段网络链路而言的。每个传输环节都有自己的MTU。数据包传输时，必须符合整条路径上最小的那个MTU(即路径MTU)。

常见数值：
- 以太网：1500字节。这是互联网和大多数局域网的标准默认值。
- PPPoE(常用的宽带拨号连接)：1492字节。比标准以太网多了8字节的PPPoE头。
- Jumbo Frame(巨型帧)：9000字节。主要用于高速数据中心内部网络，需要所有设备支持。

数据太大的解决办法——分片  
当发送方要发送的数据大于下一跳的MTU时，分片机制就会启动。  
- 操作系统或网络设备会将一个大的IP数据包切割成多个小于等于MTU的小数据包
- 
...
- 快速重传与快速恢复：
	...
	- 快速恢复：
		- 在快速重传后，将慢启动阈值设置为cwnd的一半`ssthresh = max(cwnd/2, 2)`
		- 将拥塞窗口设置为慢启动阈值加上三个MSS `cwnd = ssthresh + 3 MSS`
...
### 超时（Timeout）
- 触发条件：重传定时器(RTO)到期, 还没有收到ACK。
- 超时认为发生了严重阻塞或数据包丢失。
- 此时TCP会将阈值设定为`ssthresh = max(cwnd/2, 2)`，同时将拥塞重置到最小窗口`cwnd = 1 MSS`，随后进入慢启动阶段。

#### 超时与快速重传区别
快速重传的触发条件是收到3个重复的ACK，此时认为发生了个别数据包丢失，但网络仍有传输能力，立刻重发数据包并进入快速恢复阶段。
...
- WebSocket允许客户端和服务器之间建立一个持久的TCP连接
...
### 订阅机制
订阅机制：假设有多个用户同时与服务器建立WebSocket连接，服务器如何协调资源；  
- 服务器连接建立后，向服务器发送"订阅文件ID"的请求，表示"我正在使用/关注这个文件"
- 服务器维护一个数据结构，记录"文件ID → 订阅该文件的客户端列表"，例如：

``` cpp
const fileSubscribers = {
  'fileX': Set<WebSocket>,  // 订阅fileX的客户端连接集合
  'fileY': Set<WebSocket>,
};
```

当文件发生修改，修改请求必须发送到服务器，由服务器串行处理。
- 顺序保证：服务器单线程或使用队列，按请求顺序处理修改。
- 版本号控制：给文件维护一个递增的版本号(或时间戳)，每次修改后版本号+1。
- 冲突检测：
	- 如果B的修改基于旧版本，服务器可以拒绝修改或要求客户端先拉取最新版本。这就是常见的**乐观锁机制**。

### 实时协同的常用技术方案
- WebSocket：服务器和客户端之间保持长连接，服务器主动推送消息。适合实时性要求高的场景。
- SSE：单向推送，服务器向客户端推送事件，客户端监听。适合通知类场景。
- 长轮询：客户端定期请求服务器，服务器有更新时返回数据。实时性较差。
...
FTP 是一种用于在网络上进行文件传输的标准协议，允许用户在客户端和服务器之间上传、下载、管理文件。

- 协议类型：应用层协议
- 标准端口：
	- 控制连接端口：21(用于传输命令和响应)
	- 数据连接端口：20(默认用于传输数据，但实际数据端口可动态分配)
- 传输层协议：基于 TCP，保证可靠传输
...
### 主动模式与被动模式
- 主动模式 (Active Mode)
	- 客户端打开一个随机端口，告诉服务器该端口号
	- 服务器从自己的20端口连接客户端指定端口建立数据连接
	- 服务器主动连接客户端

- 被动模式 (Passive Mode)
	- 客户端告诉服务器进入被动模式
	- 服务器打开一个随机端口，告诉客户端该端口号
	- 客户端连接服务器指定端口建立数据连接
	- 客户端主动连接服务器
...
2. 被动模式适合在防火墙或NAT后面
	主动模式是服务器主动连接客户端指定端口，在防火墙或NAT(网络地址转换)后面，外部服务器发起的连接请求很可能被阻止。
	被动模式是客户端发起连接请求，由防火墙和NAT允许内部主机发起的连接。

3. 21端口是控制连接，主动模式和被动模式都需要；20端口主动模式需要，被动模式由服务器打开一个随机高端口(通常在1024以上)
...
### 安全性问题
- FTP明文传输用户名、密码和数据，存在安全风险
- 现代环境中，通常使用FTPS(FTP Secure)或SFTP(SSH File Transfer Protocol)替代
...
### FTP和WebSocket传输文件的区别
- FTP通常需要专门的FTP客户端软件(如FileZilla、WinSCP等)来连接FTP服务器。
- WebSocket基于HTTP/HTTPS，浏览器原生支持WebSocket，开发者可以在网页中用JavaScript直接建立WebSocket连接。
...
SSE 是一种由浏览器原生支持的 单向实时推送技术，允许服务器主动向客户端推送文本消息。

### 工作原理
1. 客户端使用 JavaScript 创建一个 EventSource 对象，向服务器发起请求。
2. 服务器保持这个 HTTP 连接不断开，持续发送文本格式的事件数据。
3. 客户端通过监听 message 事件接收服务器推送的数据。
4. 如果网络断开，浏览器会自动尝试重连。

### 适用场景
- 实时通知：当服务器检测到某个资源(如文件、文档)被修改时，立即向所有相关客户端推送更新通知。
- 数据同步：客户端接收到通知后，可以主动请求最新数据，实现数据的同步更新。
- 状态广播：例如多人编辑时，广播用户的状态变化、光标位置、编辑操作等。

## 负载均衡 (Load Balancing)
负载均衡是指将用户的请求或任务，按照一定策略分配到多台服务器或资源节点上，以实现资源的合理利用、提高系统的响应速度和可靠性。

### 作用
- 提高系统性能：多个服务器分担请求压力，提高整体处理能力。
- 增强系统可靠性：某台服务器故障时，流量自动切换到其他服务器，保证服务不中断。
- 实现高可用架构：支持系统冗余和弹性扩展。
- 优化资源利用率：避免部分服务器空闲，部分服务器过载。

### 工作原理
1. 客户端请求到达负载均衡器
负载均衡器作为请求入口，接收所有客户端请求。
2. 选择目标服务器
根据预设的负载均衡算法，选择一台或多台服务器来处理请求。
3. 请求转发
将请求转发到选中的服务器。
4. 响应返回客户端
服务器处理请求后，响应通过负载均衡器返回给客户端。

### 常见算法
- 轮询(Round Robin)
按顺序将请求依次分配给服务器。
- 加权轮询(Weighted Round Robin)
根据服务器性能分配权重，性能好的服务器获得更多请求。
- 最少连接数(Least Connections)
将请求分配给当前连接数最少的服务器。
- 源地址哈希(IP Hash)
根据客户端IP地址计算哈希值，固定分配到某台服务器，保证会话一致性。
- 随机(Random)
随机选择服务器。

## ARP (Address Resolution Protocol)
ARP协议是一种网络协议，用于将网络层的地址（IPv4地址）解析为数据链路层的硬件地址（MAC地址）。它工作在局域网中，帮助主机找到目标主机的物理地址，从而实现数据帧的正确发送。

### 工作流程
1. 发送ARP请求
当主机A需要发送数据给主机B，但不知道B的MAC地址时，主机A会广播一个ARP请求包，内容包括:
- 发送者的IP和MAC地址
- 目标的IP地址
- 目标的MAC地址字段填0（未知）

2. 接收ARP请求
网络中所有主机都会接收到ARP请求，但只有目标IP地址匹配的主机会响应。

3. 发送ARP响应
目标主机B收到请求后，发送一个ARP响应包，告诉主机A自己的MAC地址。

4. 缓存MAC地址
主机A收到响应后，将IP-MAC映射缓存起来，供后续使用。

### 特点
- 无连接协议，基于广播请求，单播响应。
- 工作在链路层和网络层之间。
- 仅限于局域网内使用，跨网段通信需要路由器(路由器不转发广播帧)。