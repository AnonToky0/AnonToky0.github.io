...
DNS通过一系列的查询，将域名转为IP地址。
	-基本流程
		...
		2. 浏览器检查本地DNS缓存是否有对应的IP地址
			如果没有，查询请求发送给本地操作系统的DNS解析器
	...
	-DNS安全
		...
		DNS over TLS(DoT)
			DoT通过专用的TLS连接来加密DNS查询和响应。它使用专用的端口(853)。
			
2. HTTP和HTTPS
	HTTP：超文本传输协议
	HTTPS:安全超文本传输协议
		都是应用层协议，广泛用于Web浏览、数据传输和在线服务。
		HTTP定义了请求和响应的格式和规则，使得不同计算机和平台之间能有效通信。
		
	HTTP默认使用80端口
	HTTP工作原理：
		1. 客户端发起HTTP请求：用户在浏览器中输入URL，浏览器解析URL，提取域名和资源路径
		2. DNS查询：递归查找：浏览器缓存->操作系统缓存->DNS解析器=>DNS服务器
			->根DNS服务器->TLD服务器->权威DNS服务器->查到IP返回
		3. 建立TCP连接，默认80端口
	...
	可能的漏洞：
		...
		2. SSL/TLS 漏洞
			1. SSL 3.0 填充模式的缺陷
			2. OpenSSL实现中的 缓冲区溢出漏洞
			3. TLS 1.0及更低版本中的块加密模式漏洞
			
		
	HTTP状态行
		响应报文的第一行(状态行)，依次是当前HTTP版本号，3位数字组成的状态码，及描述状态的短语
	HTTP状态码
		1XX: 请求已被接收，继续处理
		2XX: 成功接收、理解和处理
		...
		5XX 服务器错误
			...
			503 Serive Unavailable:		服务器暂时无法处理请求，通常由于过载或维护
			504 Gateway Timeout:		服务器作为网关或代理时，上游服务器未及时响应
	...
	HTTP常见字段：
		...
		响应头字段：
			...
			Content-Type:		响应主体的媒体类型， 如 Content-Type: text/html; charset=UTF-8
...
3. IP协议
	...
	IPv4地址采用32位，通常以点分十进制表示。 		4个字节
	IPv6地址采用128位，使用冒号分隔的十六进制形式	16个字节
...
4. UDP和TCP
	...
	TCP头部格式
		...
		序列号：在建立连接时由计算机生成的随机数作为初始值，通过SYN包传给接收端主机。每发送一次就累加该大小。
			用来解决网络包乱序问题
	...
	UDP(User Datagram Protocol)用户数据报协议，位于传输层，无连接、不可靠
		目标是提供快速、低延迟的数据传输
		特性：
...
6. 拥塞控制
	...
	主要拥塞控制算法有：慢启动、拥塞避免、快速重传与快速恢复、拥塞窗口与慢启动阈值
		...
		慢启动：
			...
			每收到一个确认应答ACK，cwnd增加一个MSS(maxinum segment size)最大报文段长度
...
7. 强缓存和协商缓存
		缓存可以：
			...
			更快加载页面
			减少服务器负载
	强缓存：浏览器判断请求的目标资源是否有效命中强缓存，命中从内存中读取目标资源，无需与服务器通讯
	...
	1. 基于last-modified的协商缓存
		...
		之后每次对该资源的请求，都带上If-Modified-Since字段，服务端拿到这个时间并读取该资源的修改时间
			做比较来决定是读取缓存还是返回新的资源
...
5. HTTP请求 常见状态码和字段
	HTTP常见字段：
		...
		-Content-length：服务器返回数据时，带有该字段表示回应的数据长度
		-Connection: 客户端要求服务器使用HTTP长连接时使用
		-Content-Type: 服务器回应时，告诉客户端本次数据的格式
		-Content-Encoding：服务器返回数据使用了什么压缩格式
