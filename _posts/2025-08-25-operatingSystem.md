...
#### 页表和MMU
虚拟地址和物理地址通过页表来映射。  
页表是一种数据结构，维护虚拟内存和物理内存之间的映射关系，或者说用来记录**虚拟页号 → 物理页框号**的映射关系。
...
2. 页表项(PTE, Page Table Entry)
页表项是页表中的最小单位，即相当于数组(页表)中通过下标(VPN)访问到的对应元素(PTE)。
...
问题是虚拟地址空间很大，这导致页表巨大，32位系统需要~4MB，引入多级页表机制。
...
不分级页表覆盖需要2^20页，而二级页表只需要1024页，二级页表在进程运行时**按需创建**。
...
### 堆(Heap)
堆是程序运行时由程序员手动管理的一块内存区域，用于动态分配内存。

#### 特点
- 动态分配和释放：程序员通过`new/delete`（C++）或`malloc/free`（C）等手动控制。
- 存储内容：动态分配的对象或数据。
- 访问速度相对较慢：堆内存分配和释放比栈复杂，效率较低。
- 大小较大：堆的大小仅受限于系统内存。
- 需要手动管理：不正确的管理会导致内存泄漏或野指针。

### 栈(Stack)
栈是一块由操作系统自动分配和管理的内存区域，主要用于存储函数的局部变量、函数参数和返回地址。

#### 特点
- 自动分配和释放：函数调用时，自动在栈上分配空间；函数返回时，自动释放。
- 存储内容：局部变量、函数参数、返回地址等。
- 访问速度快：由于栈内存连续且管理简单，访问速度快。
- 大小有限：栈的大小通常较小（几MB），过多的递归或大量局部变量可能导致栈溢出（stack overflow）。
- 先进后出（LIFO）：栈遵循后进先出原则。
...
| 管理方式       | 由操作系统管理                     | 由程序员管理                       |
| 访问速度       | 快                                | 相对较慢                          |
| 生命周期       | 随函数调用自动创建和销毁           | 由程序员控制，直到手动释放或程序结束 |
| 空间大小       | 较小，有限                        | 较大，受系统内存限制               |
| 存储内容       | 局部变量、函数参数、返回地址       | 动态分配的对象或数据               |
| 分配方式       | 连续内存块，先进后出               | 不连续，碎片化可能性大             |
| 典型错误       | 栈溢出（Stack Overflow）           | 内存泄漏、野指针                   |
...
设计目的是 充分利用内存空间，避免冲突。
当堆和栈相遇时，说明内存耗尽（栈溢出或堆空间不足）。

> 代码段、数据段(.data, .bss)位于堆地址的下面。
> 注意，这里说的地址分配都是基于虚拟地址空间说明的。
...
### 调用栈
调用栈是计算机内存中的一块特殊区域，用于动态追踪程序执行到何处以及每个函数调用的上下文信息。

#### 核心工作原理与结构
1. 栈帧：每次函数被调用时，都会在调用栈顶部压入一个新的栈帧。
2. 栈帧的内容：
    - 局部变量：函数内部定义的变量。
    - 函数参数：调用时传入的参数值
    - 返回地址：函数执行完毕后，应该回到调用者的哪条指令继续执行。
    - 前一个栈帧的指针：用于在函数返回后，能恢复上一个函数的上下文。
3. 函数返回：当函数执行完毕时，它的栈帧会从栈顶弹出。程序会根据弹出的栈帧中的“返回地址”，跳转回调用者函数继续执行，并且调用者可以访问该函数的返回值。

#### 参数传递
参数传递机制本质上是通过调用约定(Calling Convention)和 栈或寄存器来实现的。  
参数传递主要有寄存器和栈两种方式，具体取决于参数大小、数量和架构的调用约定。

##### 调用约定
调用约定定义了函数调用时参数如何传递、返回值如何传递、调用者和被调用者的职责(如保存哪些寄存器)。常见调用约定有：
1. cdecl(C声明)：参数从右向左压栈，调用者负责清理栈。
2. stdcall：参数从右向左压栈，被调用者负责清理栈。
3. fastcall：部分参数通过寄存器传递，提高效率。

例子(以cdecl为例)：  
1. 调用者准备函数  
按照调用约定，从右向左将参数依次压入栈中。  
2. 调用指令  
调用指令(如 call)将程序计数器(PC)跳转到函数地址，并将返回地址压入栈。  
3. 函数执行
    - 函数在栈上建立栈帧(frame)，保存返回地址、旧的基地址(EBP/RBP)等信息
    - 函数通过栈指针(ESP/RSP)和基指针访问参数和局部变量。例如，参数通过基指针加偏移访问。
4. 函数返回值
函数执行完毕后，将返回值放入约定的寄存器(如x86的EAX)中。  
5. 调用者清理栈
调用者根据调用约定调整栈指针，清理参数占用的栈空间。

##### 物理实现
1. 寄存器传递(现代常用，速度快)
对于少量、较小的参数(如整数、指针)，CPU寄存器是首选方式。

2. 栈传递(传统方式或参数过多时)

``` c
// 函数调用: bar(1,2,3,4,5,6,7,8)
// 在x86-64上，前6个通过寄存器，后2个通过栈：
// 栈顶(低地址)
// | 返回地址 | <-- 调用时自动压入
// | 参数7    | <-- 调用者压入
// | 参数8    |
...
- 每个进程拥有独立的内存空间(堆、栈、数据段)、进程控制块(PCB)、打开的文件描述符表、信号处理和信号屏蔽字
...
### 崩溃如何解决
1. 定位崩溃原因
- 使用调试工具（如 gdb、Visual Studio 调试器）定位崩溃发生的代码位置。
- 查看崩溃日志、错误码、堆栈信息。
- 复现崩溃场景，捕获崩溃时的上下文信息。
2. 分析根本原因
3. 修复代码
...
## 硬链接(Hard Link)
在类Unix文件系统中，硬链接是一种多个文件名指向同一个文件数据的机制。

- 一个文件的数据本体存放在磁盘的某个位置
- 这个数据由一个inode(索引节点)描述
- 硬链接让多个目录项同时指向同一个inode

### ln
ln 是 类Unix 系统（Linux / macOS） 中用于**创建链接（link）**的命令，可以创建 硬链接 或 软链接（符号链接）。
...
``` bash
ln a.txt b.txt   # b.txt 是 a.txt 的硬链接
```
...
### 硬链接特性
- 数据完全共享，修改任意一个文件名，其他硬链接看到的内容也会改变
...
- 删除文件 ≠ 删除数据
    ...
    - 只是删除了一个目录项
    - inode 的 link count 减 1
    - 只要 link count > 0，数据仍然存在
...
2. 可以跨文件系统
``` bash
ln -s /mnt/data/file /home/user/file_link
```
